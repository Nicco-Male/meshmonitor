...
export interface TracerouteCampaignDependencies {
  ...
  authorize(ownerId: number, sourceId: string, channel: number): Promise<boolean>;
...
/** Result visibility follows the upstream traceroute source/channel gates. */
export async function canAccessCampaignSource(
  user: User,
  sourceId: string,
  action: 'read' | 'write',
  channel?: number,
): Promise<boolean> {
  if (!await hasPermission(user, 'traceroute', 'read', sourceId)) return false;
  if (action === 'write' && !await hasPermission(user, 'traceroute', 'write', sourceId)) return false;
  return channel === undefined
    || (await maskTraceroutesByChannel([{ channel }], user, sourceId)).length === 1;
}

type WaitOutcome = { kind: 'success'; trace: Partial<DbTraceroute> }
  | { kind: 'timeout' } | { kind: 'cancelled' } | { kind: 'error'; error: string };
...
export function validateTracerouteCampaignInput(input: CreateTracerouteCampaignInput): CreateTracerouteCampaignInput {
  ...
  if (input.targets.some(target => !target || typeof target !== 'object'
    || (typeof target.nodeNum !== 'number' && typeof target.nodeNum !== 'string'))) {
    throw new TracerouteCampaignError('Every target must have a valid unicast node number', 400, 'INVALID_CAMPAIGN_TARGET');
  }
  if (input.sourceIds.some(id => typeof id !== 'string' || !id.trim())) {
    throw new TracerouteCampaignError('Every source must have a non-empty id', 400, 'INVALID_CAMPAIGN_SOURCE');
  }
...
export class TracerouteCampaignService {
  ...
  async create(input: CreateTracerouteCampaignInput, ownerId: number): Promise<TracerouteCampaign> {
    ...
    try {
      const config = validateTracerouteCampaignInput(input);
      const sources = await this.resolveSources(config.sourceIds, ownerId);
      ...
      for (const target of config.targets) {
        const priorities = await Promise.all(sources.map(async (source, selectedOrder) => {
          const traces = await this.deps.getRecentTraceroutes(source.id, source.localNodeNum, target.nodeNum, cutoff, source.channel);
  ...
  async retry(id: string, ownerId: number, isAdmin = false): Promise<TracerouteCampaign | null> {
    ...
    try {
      ...
      const sources = await this.resolveSources(sourceIds, ownerId);
  ...
  private async resolveSources(sourceIds: string[], ownerId: number): Promise<TracerouteCampaignSource[]> {
    ...
    return Promise.all(sourceIds.map(async (sourceId) => {
      ...
      if (!await this.deps.authorize(ownerId, sourceId, channel)) {
        throw new TracerouteCampaignError('Insufficient campaign source or channel permission', 403, 'FORBIDDEN');
      }
      return { id: source.id, name: source.name, localNodeNum, channel };
    }));
  }
  ...
  private launchReservedCampaign(campaign: TracerouteCampaign): void {
    ...
    void this.run(campaign.id)
      ...
      .finally(() => {
        this.deps.releaseSources(campaign.id);
  ...
  private async runJob(campaign: TracerouteCampaign, job: TracerouteCampaignJob): Promise<void> {
    ...
    if (localNodeNum !== job.localNodeNum) {
      this.finishJob(job, 'error', 'The source local node changed before this attempt');
      return;
    }
    ...
    let waiter: ReturnType<TracerouteCampaignService['waitForTraceroute']> | undefined;
    const cancel = () => {
      controller.abort();
      waiter?.cancel();
      resolveCancellation(cancelled);
    };
    this.cancelCurrentWait = cancel;
    const timeoutMs = campaign.config.timeoutSeconds * 1000;

    try {
      const channel = await Promise.race([this.deps.resolveChannel(manager), cancellation]);
      if (channel === cancelled) {
        this.finishJob(job, 'cancelled', 'Campaign cancelled');
        return;
      }
      const authorized = await Promise.race([
        this.deps.authorize(campaign.ownerId, job.sourceId, channel), cancellation,
      ]);
      if (authorized === cancelled) {
        this.finishJob(job, 'cancelled', 'Campaign cancelled');
        return;
      }
      if (!authorized) throw new Error('Campaign source or channel permission was revoked');
      job.channel = channel;
      waiter = this.waitForTraceroute(job.sourceId, localNodeNum, job.target.nodeNum, channel, timeoutMs);
      const dispatch = manager.sendCampaignTraceroute(
        job.target.nodeNum, channel, campaign.retryOfCampaignId ? 'retry' : 'campaign', timeoutMs,
        () => !controller.signal.aborted && campaign.status !== 'cancelled',
        {
          signal: controller.signal,
          onDispatch: async () => {
            if (!await this.deps.authorize(campaign.ownerId, job.sourceId, channel)) {
              throw new Error('Campaign source or channel permission was revoked');
            }
            if (controller.signal.aborted) throw new Error('Campaign cancelled');
            job.status = 'running';
      ...
      const sent = await Promise.race([dispatch, cancellation]);
      if (sent === cancelled) {
        this.finishJob(job, 'cancelled', 'Campaign cancelled');
        return;
      }
      ...
      if (outcome.kind === 'success') {
        ...
        this.finishJob(job, 'error', outcome.error);
      } else {
      ...
      waiter?.cancel();
      if (this.cancelCurrentWait === cancel) this.cancelCurrentWait = null;
    }
  }
  ...
  private waitForTraceroute(
    ...
    let finish!: (result: WaitOutcome) => void;
    let settled = false;
    ...
    const promise = new Promise<WaitOutcome>(resolve => {
      ...
      unsubscribe = this.deps.subscribe(event => {
        if (!dispatched || event.sourceId !== sourceId) return;
        if (event.type === 'connection:status' && (event.data as { connected?: boolean })?.connected === false) {
          finish({ kind: 'error', error: 'Source disconnected while waiting for a response' });
          return;
        }
        if (event.type !== 'traceroute:complete') return;
        const trace = event.data as Partial<DbTraceroute> | null;
        if (!trace || (trace.channel != null && trace.channel !== channel)) return;
    ...
    return {
      promise, cancel: () => finish({ kind: 'cancelled' }),
      enable: () => { dispatched = true; }, armTimeout,
    };
  }
...
const defaultDependencies: TracerouteCampaignDependencies = {
  ...
  getRecentTraceroutes: async (sourceId, localNodeNum, targetNodeNum, sinceTimestamp, channel) => {
  ...
  authorize: async (ownerId, sourceId, channel) => {
    const user = await databaseService.findUserByIdAsync(ownerId);
    return !!user?.isActive && canAccessCampaignSource(user, sourceId, 'write', channel);
  },
