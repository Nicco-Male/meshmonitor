  ...
  const nodeMarkers: NodeMarkerDescriptor[] = visibleMapNodes
    .map(node => {
      ...
      return {
        key: markerKey,
        position,
        iconSig: `${node.nodeNum}-${hops}-${isSelected}-${node.user?.role}-${node.mobile === 1 || node.isMobile === true ? 1 : 0}-${node.isUnmessagable ? 1 : 0}-${node.user?.shortName}-${showLabel}-${shouldAnimate}-${showRoute && isSelected}-${mapPinStyle}-${mapPinColorMode}-semantic-role-v1`,
        buildIcon: () =>
          createNodeIcon({
            variant: 'meshtastic',
            hops,
