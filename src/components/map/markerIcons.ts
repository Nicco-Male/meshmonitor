...
export function createNodeIcon(options: CreateNodeIconOptions): L.DivIcon {
  ...
  if (pinStyle === 'official') {
    ...
    const useSemanticRoleColor = semanticRoleColor && !fixedColor;
    const semanticRoleAccent = useSemanticRoleColor
    ...
    const fillColor = fixedColor
      ?? (useSemanticRoleColor
        ? null
        : colorMode === 'hops'
          ? getHopColor(hops)
          : (nodeNum != null ? meshtasticNodeColor(nodeNum) : null));
    const textFill = useSemanticRoleColor ? '#333' : (fillColor ? readableTextColor(fillColor) : '#333');
    const textHalo = useSemanticRoleColor
      ? '#ffffff'
      : (fillColor ? (textFill === '#ffffff' ? '#000000' : '#ffffff') : '#ffffff');
    const circleFill = useSemanticRoleColor ? 'white' : (fillColor ?? 'white');
    const circleFillOpacity = useSemanticRoleColor ? '0.95' : (fillColor ? '1' : '0.95');
    const circleStroke = useSemanticRoleColor ? outlineColor : (fillColor ? '#ffffff' : color);
