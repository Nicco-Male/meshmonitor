...
describe('TracerouteCampaignPanel', () => {
  beforeEach(() => {
    apiMocks.get.mockReset().mockImplementation((path: string) => {
      if (path === '/api/traceroutes/scheduler/status') {
        return Promise.resolve({ maxActive: 1, cooldownMs: 0, active: null, queue: [] });
      }
  ...
  it('shows the successful path details and retries only failed attempts', async () => {
    ...
    apiMocks.get.mockImplementation((path: string) => {
      if (path === '/api/traceroute-campaigns/active') return Promise.resolve({ campaign: null });
      if (path === '/api/traceroute-campaigns/latest') return Promise.resolve({ campaign: completed });
      if (path === '/api/traceroutes/scheduler/status') {
        return Promise.resolve({ maxActive: 1, cooldownMs: 0, active: null, queue: [] });
      }
